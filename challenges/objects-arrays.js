...
const universities = graduates.map(Obj => Obj.university);
...
const contactInfo = graduates.map(Obj => `${Obj.first_name} ${Obj.email}`);
...
const unisWithUni = graduates.filter((obj) => {
  return obj.university.includes('Uni');
});
...
const lowCaseAnimalNames = zooAnimals.map((Obj) => {
...
*/
const populationTotal = zooAnimals.reduce((sum, Obj) => {
